...
void LandmarkGraph::print_landmarks() {
    std::vector<std::pair<int,int> > landmarks;
    set<LandmarkNode *, LandmarkNodeComparer> nodes2(nodes.begin(), nodes.end());
    for (set<LandmarkNode *>::const_iterator it = nodes2.begin(); it != nodes2.end(); it++)
    {
        LandmarkNode *node_p = *it;
        int var_no = node_p->vars[0];
        int value = node_p->vals[0];
        cout << g_fact_names[var_no][value] << endl;
    }
}


std::vector<std::pair<int,int> > LandmarkGraph::extract_landmarks() {
    std::vector<std::pair<int,int> > landmarks;
    set<LandmarkNode *, LandmarkNodeComparer> nodes2(nodes.begin(), nodes.end());
    for (set<LandmarkNode *>::const_iterator it = nodes2.begin(); it != nodes2.end(); it++)
    {
        ...
        // int var_no = node_p->vars[0];
        // int value = node_p->vals[0];
        // std::pair<int,int> var_value = make_pair(var_no, value);
        landmarks.push_back(make_pair(node_p->vars[0], node_p->vals[0]));
        // int var_no = node_p->vars[0], value = node_p->vals[0];
        // cout << g_fact_names[var_no][value] << endl;
    }
    return landmarks;
}
