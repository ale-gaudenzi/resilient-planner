...
typedef std::pair<State, std::vector<const Operator *> > state_plan_pair; // k-v pair is useful for indexing the resilient policies and the policies generated. 
...
extern std::set<Operator> g_current_forbidden_ops; // set of operators that are forbidden at the current node
extern std::map<k_v_pair, Policy *> g_resilient_policies; // map of resilient policies indexed by the number of faults and the set of forbidden ops
extern std::map<k_v_pair, Policy *> g_non_resilient_deadends; // map of forbidden <state, operator> pairs indexed by the number of faults and the set of forbidden ops
extern Policy *g_original_policy; // first policy generated by the planner
extern std::vector<State> g_dead_states; //fast test for deadends states
extern std::map<std::string, state_plan_pair > g_safe_states; //fast test for deadends states
extern std::vector<std::string> safe_actions;
