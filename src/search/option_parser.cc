    ...
    int argc, const char **argv, bool dry_run)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (arg.compare("--plan-to-file") == 0)
        {
            ++i;
            g_plan_to_file = (atoi(argv[i]) == 1);
        }
        else if (arg.compare("--dump-branches") == 0)
        {
            ++i;
        ...
        else if (arg.compare("--search_pruning") == 0)
        {
            ++i;
                                   ...
                                   const OptionFlags &flags)
{
    ...
    if (!(str_stream >> x).fail())
    {
        ...
        {
            error("invalid enum argument " + name + " for option " + k);
        }
        opts.set<int>(k, x);
    }
    else
...
Options OptionParser::parse()
{
         ...
         pti != end_of_roots_children(parse_tree); ++pti)
    {
        if (pti->key.compare("") != 0)
        {
            ...
            if (!valid_key)
            {
                error("invalid keyword " + pti->key + " for " + parse_tree.begin()->value);
            }
        }
        if (pti->key.compare("") == 0 &&
            last_key.compare("") != 0)
