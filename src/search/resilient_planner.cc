...
std::list<ResilientNode> not_resilient_nodes;
...
bool resiliency_check(ResilientNode node)
{
    ...
    for (std::set<Operator>::iterator it_o = next_actions.begin(); it_o != next_actions.end(); ++it_o)
    {
        ...
        if ((find_in_nodes_list(resilient_nodes, successor_r) || (*goal_step->state).is_implied(successor_p)) && (find_in_nodes_list(resilient_nodes, current_r)))
        {
